...
```{r}
ggplot(mergedB, aes(x = ABV)) + geom_histogram() + ggtitle("ABV Histogram")
summary(mergedB$ABV)
```
...
mergedBall %>% ggplot(aes(x = IBU, y = ABV, color = group)) + geom_point() +
  ggtitle("Scatterplot: ABV vs. IBU")
...
ggplot(df, aes(x = reorder(State, -meanABV), y = meanABV)) + geom_bar(stat = "identity")+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +ggtitle("Avg. ABV By State(Ale)")
...
ggplot(df, aes(x = reorder(State, -meanABV), y = meanABV)) + geom_bar(stat = "identity")+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ggtitle("Avg. ABV By State(IPA)")
```
